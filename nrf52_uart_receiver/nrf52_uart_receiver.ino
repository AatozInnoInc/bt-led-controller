...
void loop()
{
  ...
  if ( Bluefruit.connected() )
  {
    ...
    if (bleuart.available()) {
      Serial.println("Data available from BLE UART!");
      
      int command = bleuart.read();
      Serial.printf("Received command: %c (ASCII: %d)\n", command, command);
      ...
      switch (command) {
        ...
        default: {   // Handle text messages (like "Hello World!")
            if (command != -1) {  // -1 means no data available
              handleTextMessage(command);
            }
            break;
        }
      }
...
void handleTextMessage(int firstChar) {
  ...
  String fullMessage = "";
  fullMessage += (char)firstChar;
  
  // Read and print the rest of the message
  int charCount = 1;
  ...
  Serial.printf("Total characters received: %d\n", charCount);
  Serial.println("=== END MESSAGE ===");
  
  // Handle different message types
  if (fullMessage == "ERROR_TEST") {
    // Send error response for testing
    bleuart.println("ERROR: Test failure triggered successfully");
    Serial.println("Sent error response for testing");
  } else if (fullMessage == "Hello World!") {
    // Send success response
    bleuart.println("SUCCESS: Hello World! message received and processed");
    Serial.println("Sent success response");
  } else {
    // Send generic acknowledgment
    bleuart.println("INFO: Message received - " + fullMessage);
    Serial.println("Sent generic acknowledgment");
  }
}
