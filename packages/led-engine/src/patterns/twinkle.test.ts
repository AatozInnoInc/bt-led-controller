...
describe('twinkle pattern', () => {
  ...
  it('lights pixels whose random value is below threshold', () => {
    ...
    twinkle(pixels, makeConfig(), 0);

    expect(pixels[0]).toEqual({ r: 255, g: 255, b: 255 });
    expect(pixels[1]).toEqual({ r: 0, g: 0, b: 0 });
    expect(pixels[2]).toEqual({ r: 255, g: 255, b: 255 });
